...
function Searchbar({ placeholder = "O que você procura?", action = "/s", name = "q", loader, platform, }: Props) {
    const id = useId();
    ...
    useEffect(() => {
        ...
        const handler = (e: KeyboardEvent) => {
            ...
            isEscapeKey ? displaySearchPopup.value = false : null;
        };
    ...
    return (<div class="w-full grid gap-8 pb-6 overflow-y-hidden lg:mt-0 animate-sliding-down shadow-header" style={{ gridTemplateRows: "min-content auto" }}>
      <form id={id} action={action} class="join py-6 px-[1rem] lg:px-[3.5rem] border-secondary-neutral-200">
        <input ref={searchInputRef} id="search-input" class="input border-0 border-b border-dark-blue join-item flex-grow focus:outline-none focus:border-primary-900 border-secondary-neutral-200 placeholder:text-paragraph-color placeholder:font-light" name={name} onInput={(e) => {
        ...
        <Button type="submit" class="join-item btn-square shadow-none border-0 border-b border-dark-blue border-secondary-neutral-200 mr-4" aria-label="Search" for={id} tabIndex={-1} onClick={() => handleSearch(products[0])}>
          {loading.value
            ? <span class="loading loading-spinner loading-xs"/>
            : <Icon id="MagnifyingGlass" size={20} strokeWidth={0.01}/>}
        </Button>
        <Button type="button" class="join-item btn-ghost btn-square hidden sm:inline-flex" onClick={() => {
            setQuery("c");
      ...
      <div class={`overflow-y-scroll px-[1rem] lg:px-[5rem]`}>
        <div class="gap-4 grid grid-cols-1 sm:grid-rows-1">
          <div class={`${hasProducts || searchInputLength > 2 ? "hidden" : ""} flex flex-col gap-6`}>
            {userSearches.value.length > 0
            ? (<>
                  <span class="font-light text-xl text-paragraph-color" role="heading" aria-level={3}>
                    Suas buscas recentes
                  </span>
                  <ul id="search-suggestion" class="flex flex-col gap-6">
                    {userSearches.value.slice(-4).map((search: string) => (<li>
                        <a href={`/s?q=${search}`} class="flex gap-4 items-center justify-between">
                          ...
                          <span>
                            <Icon id="SearchAgain" size={24} strokeWidth={0.01}/>
                          </span>
                        </a>
                      </li>))}
                  </ul>
          ...
          <div class={`${!hasProducts || searchInputLength < 2 ? "hidden" : ""} flex flex-col pt-6 md:pt-0 gap-6 overflow-x-hidden`}>
            {hasProducts
            ? (<>
                  <span class="font-light text-xl text-paragraph-color" role="heading" aria-level={3}>
                    Sugestões para você
                  ...
                  <Slider class="carousel flex-col gap-4 lg:gap-0">
                    {products.map((product: Product, index: number) => (<Slider.Item index={index} class="carousel-item min-w-[200px]" onClick={() => handleSearch(product)}>
                        <ProductCardHeader product={product} index={index} itemListName="Suggeestions"/>
                      </Slider.Item>))}
                  </Slider>
                </>)
