...
export default function Benefits({ url, items = defaultValues }: SectionProps<typeof loader>) {
    const id = useId();
    const isProductPage = url.searchParams.has("skuId");
    return (<div id={id} class={`lg:container mx-6 lg:mx-auto lg:mb-20 grid gap-8 lg:gap-4 grid-cols-1 place-items-center lg:grid-cols-3 text-center px-4 ${isProductPage ? "mt-28 lg:mt-28 mb-11" : "mb-11 mt-[55px]"}`}>
      {items.map((item) => (<div class="flex flex-col items-center justify-center max-w-72">
