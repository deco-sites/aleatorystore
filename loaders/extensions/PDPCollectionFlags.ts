...
import { MasterDataSCEntity } from "./ProductShelfCollectionsFlags.ts";
...
export default function loader(_props: unknown, _req: Request, ctx: AppContext): ExtensionOf<ProductDetailsPage | null> {
  return async (productDetailsPage: ProductDetailsPage | null) => {
    ...
    const productCollectionsIds = productDetailsPage.product.additionalProperty?.filter((property) => property.name === "cluster")?.map((property) => property.propertyID);
    ...
    productDetailsPage.product.additionalProperty = productDetailsPage.product.additionalProperty ? [...productDetailsPage.product.additionalProperty, ...newProperties] : newProperties;
